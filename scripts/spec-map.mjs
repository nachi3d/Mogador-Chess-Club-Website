/**
 * Which chromium specs cover which source paths â€” the ONE mapping.
 *
 * â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 * âš ï¸ IT LIVES HERE BECAUSE TWO SCRIPTS NEED IT, AND TWO COPIES WOULD DRIFT.
 *
...
export const SPEC_MAP = [
  ...
  /* âš ï¸ An exercise now feeds the FILTER routes as well as the index: its
  ...
  /* The public agenda. âš ï¸ The git collection is GONE â€” sessions come from the
  ...
  /* E3 â€” the ledger, the ranks, the streak and the achievements. The policy
  ...
  /* âš ï¸ `supabase.ts` now carries `deleteOwnAccount()`, so the erasure spec is
     mapped from it as well â€” the one path on this site that cannot be undone
     must not be able to change without its test running. */
  /* âš ï¸ AND `markOnboarded()` â€” the first-run screen is the only caller, and the
  ...
  /* ⚠️ AND SINCE 0015 IT CARRIES THE PSEUDO PATH: registration, password
  ...
  /* The pseudo rules the browser mirrors from migration 0015. A change here
  ...
  /* The welcome screen, its two routes, and the callback that decides between
     `/bienvenue/` and `/compte/`. âš ï¸ `family.spec.ts` is here because skipping
     onboarding must leave the family section doing the whole job â€” guidance,
     not a gate â€” and that claim spans the two files. */
  [
  ...
  /* 0005 â€” the parent/child model. `child.ts` decides WHO progress belongs to,
  ...
  [
    ...
    [
      ...
      /* âš ï¸ BOTH, and the split is the point: `child-profiles` proves the RLS
  ...
  /* The sign-up form. âš ï¸ `auth.spec.ts` covers the honeypot, and the honeypot's
  ...
  [
    ...
    [
      ...
      'progress-sync.spec.ts',
      /* v2-S4 part 2 â€” the admin surfaces read every one of these tables. */
      'admin.spec.ts',
      ...
      'booking-ui.spec.ts',
      /* ⚠️ 0015 WRITES `auth.users` AND `auth.identities` DIRECTLY, which is
  ...
  /* The recurrence expansion is pure and its spec runs with no credentials and
     in both flag shapes â€” so a change to it is always covered. */
  [/^src\/lib\/recurrence\./, ['recurring-sessions.spec.ts', 'admin.spec.ts']],

  /* v2-S4 part 2 â€” the admin surfaces.
     âš ï¸ `ledger.ts` reaches BOTH specs on purpose: `progression.spec.ts` covers
  ...
  [
    ...
    [
      ...
      /* âš ï¸ The session writers live here, and "one statement, one rebuild" is a
  ...
  /* âš ï¸ The sessions page carries the attendance marker, whose SPEED is the
     design constraint of the whole feature â€” so a change to it re-measures
  ...
  /* âš ï¸ THE BAR AND THE TRAIL ARE ONE FEATURE (M4). The bar says which SECTION
     ...
     I" drift apart â€” which is the state M4 found the site in. */
  ...
  /* E2 â€” the sound module and everything that can make one. `feel.spec.ts`
  ...
  /* âš ï¸ The site header mapped to NOTHING until the desktop-progress-nav
     session, so editing the navigation selected no specs at all â€” which is
  ...
  /* âš ï¸ THE LEGAL NOTICE AND THE FACADE ARE ONE FEATURE. `#video` on
  ...
  /* The video facade â€” the component, its stylesheet and the poster pipeline.
     âš ï¸ `pwa.spec.ts` rides along on the poster script: posters are files in
  ...
  /* âš ï¸ THE FIXTURE GATE. `isListed` keeps test content off every index and
     every count, so a change here can put a fixture on `/pieges/` or make the
     trap count on `/apprendre/` one too high â€” neither of which looks wrong on
  ...
  /* Both consume the fixture predicates â€” the index draws the cards and the hub
 ...
 *
 * Returns only specs that exist on disk, sorted â€” a mapping entry for a spec
...
export function specsFor(files, root) {
  ...
  for (const file of files) {
    /**
     * âš ï¸ A SPEC YOU EDITED RUNS. This was missing until the
     ...
     *
     * Only top-level `tests/e2e/*.spec.ts` â€” a change under `helpers/` has no
