...
| Item | Status | Note |
|---|---|---|
| **⚠️ 31.9 KB of switched-off code is precached** | `soon` | Found while verifying the v0.11.0 artefact. **Not a leak** — no project ref, no anon key, no `createClient`, and the `supabase.disabled` alias is working exactly as designed. But Astro collects a page's `<script>` blocks from the **module graph, not from what renders**, so the scripts for the nine unemitted routes are still chunked *and* swept into the Workbox precache: measured **31.9 KB raw across 13 files** (`admin.js` 3.4, the four admin page scripts 12.7, `AdminShell` js+css 6.4, `AccountPage` 2.3, `ChildPicker` 1.9, `child.js` 2.2, `LoginPage` 1.4, `supabase.disabled` 0.7). ⚠️ **This is the same mechanism as the 216 KB leak the alias was written to fix** — the alias cut the client out and left the callers behind, and v2-S4 part 2 roughly doubled what is left. Every first visit on Essaouira mobile data pays for it. Likely fix: a `globIgnores` pattern in `scripts/build-sw.mjs` for the auth/admin chunks while the flag is off. ⚠️ **NOT a quick change** — it touches the service worker, so it takes the full gate. |
