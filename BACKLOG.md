...
| **⚠️ The release matrix goes red on this machine, and it is getting worse** | `soon` | Two consecutive gates now: **v0.11.0 — 4 failures in 43.9m; v0.11.1 — 7 failures in 58.3m.** Every one was a bare timeout (`Test timeout`, `Tearing down "context"`, `browser.newContext`, `locator.click`) with **not one failed assertion**, spread across firefox, webkit and iphone-13, landing on different specs each run — and every one passed a serial re-run (v0.11.1: 335 tests green across the three projects). So the diagnosis holds each time and the promotions were sound. ⚠️ **But the trend is the problem, not the individual runs.** The suite has grown past what this machine will run at the configured fan-out, and a gate that is expected to be red trains the next session to wave failures through — which is precisely how a real regression ships. Options, cheapest first: drop the global worker count for the matrix; give firefox and webkit `fullyParallel: false` as well as their retry; or split the matrix into per-project runs. ⚠️ **Do not "fix" it by loosening the timeouts** — that was tried for `play.spec.ts` and made the failure count go UP. |
