...
| **Turn accounts back on** | `seàn` | ⚠️⚠️ **MIGRATIONS FIRST, AND THIS IS NOW A HARD PREREQUISITE, NOT A TIDINESS NOTE.** As of v0.13.0 production is missing **0008** and **0009**, and the second one is load-bearing the moment the flag flips: `getProfile()` selects `onboarded_at`, so against a database without that column **every profile read fails** — the account page renders with no name and no role, the callback cannot decide where to land, and `/bienvenue/` never lets anybody past. `/admin/comptes/` would fail outright too (`admin_list_accounts` and `account_deletions` do not exist there). ⚠️ **Also outstanding: the `schema_migrations` backfill in [`docs/ADMIN.md`](./ADMIN.md)** — production's ledger still lists `0001,0002`, so a `supabase db push` would try to replay 0003–0007 including 0005's unguarded `drop constraint`. **The order is: backfill the ledger → apply 0008 and 0009 → verify with the per-migration queries in [`docs/reference/supabase.md`](./reference/supabase.md) → only then set the variable.** Then set `PUBLIC_AUTH_ENABLED=true` in the Cloudflare build variables. ⚠️ **No longer blocked — v2-S3 shipped, so there is now something to sync.** The reason accounts were off (an account that does nothing is a child's email address collected for no reason) has gone. One variable, no code, and the database is already ahead of the site, which is the safe ordering. Seàn's call. ✅ **The parent/child blocker is CLEARED — migration 0005 shipped**, so an account opened now is already a parent account holding one child and needs no later rescue. See "Modèle parent + profils enfants" below. See CLAUDE.md → "Accounts are switched off in production". |
