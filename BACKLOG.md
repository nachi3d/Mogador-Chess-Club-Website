...
| **⚠️ Author FENs need a claim-level check, not just a legality check** | `open` | Batch 3's four broken positions were all **legal, six-field, solvable** — `check-content.mjs` passed every one. They were wrong only in that the prose described a mechanism the position did not contain (a pin that was blocked, a discovery off the diagonal, a fork the defender could capture, a mate that could be blocked). No plausible extension of the checker catches that class in general; what *would* help is a convention that every board's central claim is asserted once in a scratch script at authoring time, the way batch 3 was. Worth deciding before batch 4. |
