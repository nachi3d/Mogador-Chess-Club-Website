...
## ⚠️ `agenda.spec.ts`'s axe checks on Firefox — the v0.16.0 gate finding

**Read when:** the Firefox project fails on `/agenda/` or `/en/agenda/`
“has no axe violations”, or you are tempted to raise a timeout to fix it.

**Signature.** A bare `browserContext.newPage` or `page.evaluate` **timeout**
— never an assertion naming a value — preceded in the log by:

```
Crash Annotation GraphicsCriticalError:
|[0][GFX1-]: RenderCompositorSWGL failed mapping default framebuffer, no dt
```

**Measured at the v0.16.0 gate, not guessed:**

| configuration | result |
|---|---|
| `--workers=3` | 2 failed (`/agenda/`, `/en/agenda/`) |
| `--workers=2` | 2 failed — **the same two** |
| `--workers=3`, `fullyParallel: false` | 1 failed |
| `--workers=3`, spec held out (`--grep-invert`) | **green**, and the shard ran 5.3 min → 3.2 min |
| `--workers=1`, that spec alone | **green, 8 passed** |

⚠️ **LOWERING THE WORKER COUNT DOES NOT FIX IT, AND THAT IS THE USEFUL PART.**
The failure tracks the number of **concurrent Firefox contexts**, not total
load — which is the same finding the WebKit note above records, on a second
browser. `fullyParallel: false` (now set on the firefox project) serialises the
tests *within* a file and halves the failures; it does not stop other spec
FILES running alongside, which is why one test still dies.

⚠️ **DO NOT RAISE THE TIMEOUT.** It was tried for the WebKit case and made
things worse: a starved browser given longer to answer is still starved.

**Unresolved.** The remaining fix is to make this spec genuinely one-at-a-time,
the way `play.spec.ts` is described as running — either a `test.describe.configure`
change, a dedicated project, or splitting the three 40-second axe runs out of
the file. **Until that lands, the Firefox project cannot be driven green at the
documented 3 workers on this machine**, and a promotion must not treat the
serial re-run as a substitute for a green matrix. See BACKLOG.
